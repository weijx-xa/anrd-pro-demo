...
const fakeColName = [
  { name: '职务', value: ["总经理", "副总经理", "经理", "副经理", "机关部门经理", "业务经理", "业务主管", "班组长", "业务员", "电信业务员", "话务员", "无"] },
  { name: '业务条线', value: ["维护条线", "市场条线", "工程条线", "综合条线", "其他"] },
  { name: '性别', value: ["男", "女"] },
  { name: '学历', value: ["初中", "技校", "中专", "职高", "高中", "大专", "大本", "研究生", "硕士"] },
...
const fakeColName2 = {
  name: '维护专业',
  value: ["生产管理", "家集客后台支撑", "家集客装维", "一体化装维", "一体化后台支撑", "非装维工作", "无"],
  children: [
    { name: "生产管理", value: ["家集客生产管理", "一体化生产管理"] },
    { name: "家集客后台支撑", value: ["家客支撑", "集客支撑", "客服专家坐席"] },
    { name: "家集客装维", value: ["家客装机", "家客维护", "家客装维一体", "集客维护"] },
    { name: "一体化装维", value: ["基站及配套、发电", "传输光缆维护", "铁塔及天馈维护", "WLAN维护", "室分及直放站维护"] },
    { name: "一体化后台支撑", value: ["一体化支撑"] },
    { name: "非装维专业", value: ["非装维专业"] },
  ],
};
const allJobs = fakeColName2.children.reduce((accumulator, currentValue) => [...accumulator, ...currentValue.value], []);
            ...
            >
              {fakeColName2.children.map(item2 => (
                <OptGroup key={item2.name} label={item2.name}>
                  {item2.value.map(item3 => <Option key={item3}>{item3}</Option>)}
                </OptGroup>
              ))}
            </Select>
          )}
          <a style={{ marginLeft: 10 }} onClick={() => setFieldsValue({ [fakeColName2.name]: allJobs })}>全选</a>
          <Divider type="vertical" />
          <a style={{ marginLeft: 10 }} onClick={() => setFieldsValue({ [fakeColName2.name]: [] })}>重置</a>
        </FormItem>
        {
          fakeColName.map(item => (
            <FormItem {...formItemLayout} key={item.name} label={item.name}>
              {getFieldDecorator(item.name, {
                ...
                >
                  {item.value.map(item2 => <Option key={item2}>{item2}</Option>)}
                </Select>
              )}
              <a style={{ marginLeft: 10 }} onClick={() => setFieldsValue({ [item.name]: item.value })}>全选</a>
              <Divider type="vertical" />
              <a style={{ marginLeft: 10 }} onClick={() => setFieldsValue({ [item.name]: [] })}>重置</a>
            </FormItem>
          ))
        }
        <Modal
